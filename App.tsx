...
const App: React.FC = () => {
  ...
  const handleDislike = (id: string) => {
    setInteractions(p => ({ ...p, dislikedIds: [...new Set([...p.dislikedIds, id])], likedIds: p.likedIds.filter(x => x !== id) }));
  ...
  return (
    <div className="min-h-screen bg-black text-white selection:bg-[#ff003c] selection:text-white">
      ...
      <main className="pt-20 max-w-lg mx-auto overflow-x-hidden">
        {currentView === AppView.ADMIN ? (
          ...
          (() => {
            ...
            switch(currentView) {
              case AppView.OFFLINE: return <Suspense fallback={null}><OfflinePage allVideos={rawVideos} interactions={interactions} onPlayShort={(v, l) => { setSelectedShort({video:v, list:l}); markAsPlayed(v.id); }} onPlayLong={(v) => { setSelectedLong({video:v, list:longsOnly}); markAsPlayed(v.id); }} onBack={() => setCurrentView(AppView.HOME)} onUpdateInteractions={setInteractions} /></Suspense>;
              case AppView.CATEGORY: return <Suspense fallback={null}><CategoryPage category={activeCategory} allVideos={activeVideos} isSaved={interactions.savedCategoryNames.includes(activeCategory)} onToggleSave={() => setInteractions(p => ({...p, savedCategoryNames: p.savedCategoryNames.includes(activeCategory) ? p.savedCategoryNames.filter(c => c !== activeCategory) : [...p.savedCategoryNames, activeCategory]}))} onPlayShort={(v, l) => { setSelectedShort({video:v, list:l}); markAsPlayed(v.id); }} onPlayLong={(v) => { setSelectedLong({video:v, list:longsOnly}); markAsPlayed(v.id); }} onBack={() => setCurrentView(AppView.HOME)} /></Suspense>;
              case AppView.TREND: return <Suspense fallback={null}><TrendPage onPlayShort={(v, l) => { setSelectedShort({video:v, list:l}); markAsPlayed(v.id); }} onPlayLong={(v) => { setSelectedLong({video:v, list:longsOnly}); markAsPlayed(v.id); }} excludedIds={interactions.dislikedIds} allVideos={rawVideos} /></Suspense>;
              case AppView.LIKES: return <SavedPage savedIds={interactions.likedIds} savedCategories={[]} allVideos={rawVideos} onPlayShort={(v, l) => { setSelectedShort({video:v, list:l}); markAsPlayed(v.id); }} onPlayLong={(v) => { setSelectedLong({video:v, list:longsOnly}); markAsPlayed(v.id); }} title="الإعجابات" onCategoryClick={(c) => { setActiveCategory(c); setCurrentView(AppView.CATEGORY); }} />;
              case AppView.SAVED: return <SavedPage savedIds={interactions.savedIds} savedCategories={interactions.savedCategoryNames} allVideos={rawVideos} onPlayShort={(v, l) => { setSelectedShort({video:v, list:l}); markAsPlayed(v.id); }} onPlayLong={(v) => { setSelectedLong({video:v, list:longsOnly}); markAsPlayed(v.id); }} title="المحفوظات" onCategoryClick={(c) => { setActiveCategory(c); setCurrentView(AppView.CATEGORY); }} />;
              case AppView.HIDDEN: return <HiddenVideosPage interactions={interactions} allVideos={rawVideos} onRestore={(id) => setInteractions(prev => ({...prev, dislikedIds: prev.dislikedIds.filter(x => x !== id)}))} onPlayShort={(v, l) => { setSelectedShort({video:v, list:l}); markAsPlayed(v.id); }} onPlayLong={(v) => { setSelectedLong({video:v, list:longsOnly}); markAsPlayed(v.id); }} />;
      ...
      {toast && <div className="fixed top-24 left-1/2 -translate-x-1/2 z-[1100] bg-[#ff003c] px-6 py-2 rounded-full font-black shadow-[0_0_20px_#ff003c] text-white text-[10px] border border-white/20 animate-in fade-in slide-in-from-top-4 tech-font uppercase tracking-widest">{toast}</div>}
      {selectedShort && <Suspense fallback={null}><ShortsPlayerOverlay initialVideo={selectedShort.video} videoList={selectedShort.list} interactions={interactions} onClose={() => setSelectedShort(null)} onLike={handleLikeToggle} onDislike={handleDislike} onCategoryClick={(c) => { setActiveCategory(c); setCurrentView(AppView.CATEGORY); setSelectedShort(null); }} onSave={(id) => setInteractions(p => ({...p, savedIds: [...new Set([...p.savedIds, id])]}))} onProgress={(id, pr) => {}} onDownload={(video) => {}} /></Suspense>}
      {selectedLong && <Suspense fallback={null}><LongPlayerOverlay video={selectedLong.video} allLongVideos={selectedLong.list} onClose={() => setSelectedLong(null)} onLike={() => handleLikeToggle(selectedLong.video.id)} onDislike={() => handleDislike(selectedLong.video.id)} onCategoryClick={(c) => { setActiveCategory(c); setCurrentView(AppView.CATEGORY); setSelectedLong(null); }} onSave={() => setInteractions(p => ({...p, savedIds: [...new Set([...p.savedIds, selectedLong.video.id])]}))} onSwitchVideo={(v) => { setSelectedLong(p => p ? {...p, video: v} : null); markAsPlayed(v.id); }} isLiked={interactions.likedIds.includes(selectedLong.video.id)} isDisliked={interactions.dislikedIds.includes(selectedLong.video.id)} isSaved={interactions.savedIds.includes(selectedLong.video.id)} onProgress={(pr) => {}} /></Suspense>}
    </div>
  );
};
