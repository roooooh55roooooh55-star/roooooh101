...
const AppBar: React.FC<AppBarProps> = ({ onViewChange, onRefresh, currentView }) => {
  ...
  const youtubeWebUrl = `https://www.youtube.com/channel/${channelId}`;
  ...
  const getBtnClass = (view: AppView, defaultColor: string, glowColor: string) => {
    ...
    return `w-10 h-10 sm:w-12 sm:h-12 rounded-xl flex items-center justify-center transition-all duration-300 border active:scale-90 ${
      ...
      : `bg-white/5 border-white/10 ${isSaveBtn ? 'text-[#ffea00]' : defaultColor} opacity-60`
    } hover:opacity-100`;
  };
  ...
  return (
    <header className="fixed top-0 left-0 right-0 h-16 sm:h-20 bg-black/95 backdrop-blur-3xl z-[150] border-b border-white/10 px-2 sm:px-6 flex items-center justify-between shadow-[0_10px_40px_rgba(0,0,0,0.9)]">
      ...
      <div className="flex items-center gap-2 sm:gap-4">
        ...
        <a 
          href={youtubeWebUrl} target="_blank" rel="noopener noreferrer"
          className="w-10 h-10 sm:w-12 sm:h-12 rounded-xl flex items-center justify-center bg-white/5 border border-white/10 text-red-600 hover:bg-red-600 hover:text-white transition-all shadow-md active:scale-90"
        >
          <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 24 24"><path d="M19.615 3.184c-3.604-.246-11.631-.245-15.23 0-3.897.266-4.356 2.62-4.385 8.816.029 6.185.484 8.549 4.385 8.816 3.6.245 11.626.246 15.23 0 3.897-.266 4.356-2.62 4.385-8.816-.029-6.185-.484-8.549-4.385-8.816zm-10.615 12.816v-8l8 3.993-8 4.007z"/></svg>
        </a>
      </div>

      {/* الزر المركزي المقلص (تم تصغيره قليلاً) */}
      <button 
      ...
      <div className="flex items-center gap-2 sm:gap-4">
        ...
        <button 
          ...
          className={getBtnClass(AppView.HIDDEN, 'text-gray-400', 'white')}
        >
          <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="2.5"><path d="M13.875 18.825A10.05 10.05 0 0112 19c-4.478 0-8.268-2.943-9.543-7a9.97 9.97 0 011.563-3.029m5.858.908a3 3 0 114.243 4.243M9.878 9.878l4.242 4.242M9.88 9.88l-3.29-3.29m7.532 7.532l3.29 3.29M3 3l18 18" /></svg>
        </button>
        ...
        >
          <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="3"><path d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04"/></svg>
        </button>
      </div>
    </header>
  );
